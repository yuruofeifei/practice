...
func (ts *testSystem) checkDatabase(expected map[string][]([]byte)) error {
	...
	for k, v1 := range expected {
		v2, ok := kvstore[k]
		if !ok {
			return errors.New(fmt.Sprintf("the db (kvstore) does not have an expected key, \"%v\"", k))
		} else if !bytesArraysEqual(v1, v2) {
			return errors.New(fmt.Sprintf("the db (kvstore) key, \"%v\", has the wrong value", k))
		}
	}

	// Check all of actual is in expected
	for k, v1 := range kvstore {
		v2, ok := expected[k]
		if !ok {
			return errors.New(fmt.Sprintf("the db (kvstore) has an unexpected key, \"%v\"", k))
		} else if !bytesArraysEqual(v1, v2) {
			return errors.New(fmt.Sprintf("the db (kvstore) key, \"%v\", has the wrong value", k))
		}
	}
...
func (ts *testSystem) runTest(numMsgs, numDeletes, timeout int, normalClients, slowClients []*testClient, slowDelay int) error {
	...
	for slowReads+normalReads < totalReads ||
		...
		case cmd := <-readChan:
			...
			strings.TrimSuffix(cmd.readMsg, "\n")
			if cmd.err != nil {
				return cmd.err
			}
		...
		case cmd := <-writeChan:
			...
			// Send a new 'chat message' every updateFreq (new string to append to key)
